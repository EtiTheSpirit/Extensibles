...
namespace HookGenExtender.Utilities {
	public static class BepInExExtensibles {
		...
		public static bool TryGetBIEHook(MirrorGenerator mirrorGenerator, MethodDef original, out BIEHookRef hook) {
			...
			if (!hookClassDef.TryGetOrigDelegateForMethod(mirrorGenerator, original, out TypeDef bieOrigMethodDef, out MethodDef bieOrigInvoke, out EventDef hookEvt)) {
				hook = default;
				return false;
			}
		...
		/// <returns></returns>
		public static bool TryGetOrigDelegateForMethod(this TypeDef hookType, MirrorGenerator mirrorGenerator, MethodDef originalMethod, out TypeDef bieOrigMethodDef, out MethodDef bieOrigInvoke, out EventDef hookEvt) {
